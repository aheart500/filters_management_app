    ...
    ? keys
        .map((key) => {
          switch (key) {
            ...
            case "workerId":
              return { title: "كود الفني", width: "70px" };
            case "worker":
              return { title: "اسم الفني", width: "150px" };
            case "customerId":
              return { title: "كود العميل", width: "70px" };
            case "customer":
              return { title: "اسم العميل", width: "150px" };
            case "numberOfWorkers":
              return { title: "عدد الفنيين المشتركين", width: "200px" };
            case "workerRatio":
              return { title: "نسبة الفني", width: "150px" };
          }
        })
        .filter((key) => key)
    : [];
...
const Penalties = () => {
  ...
  }>(GET_FEES, { variables: { search, type: "Installing" } });
    ...
    <div>
      <Header
        ...
        buttons={[
          ...
          { title: "إضافة عمولة تركيب جديدة", link: "/installationFees/new" },
        ]}
      />
      ...
      {!loading && fees && (
        <Table
          ...
          heads={keys}
          withoutId={true}
          linkBeginning="/installationFees"
          loadMore={loadMore}
        />
      )}
    </div>
